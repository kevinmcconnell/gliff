...
impl<T: Clone> Dpb<T> {
    ...
    // 8.2.5.3
    /// Outputs pictures while more of them wait for output than the stream's
    /// reorder window allows (C.4.5.3 applied with `max_num_reorder_frames`).
    pub fn bump_over_reorder_limit(&mut self) -> Vec<Option<T>> {
        let mut pics = vec![];
