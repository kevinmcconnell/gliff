//! Spawn `gliff-server --stdio` over ssh and expose its stdio as a stream.
...
pub struct SshTarget {
    ...
    pub host: String,
    /// Remote `gliff-server` path (default: `gliff-server` on PATH).
    pub server_bin: String,
    /// Extra args passed to the server after `--stdio`.
    pub server_args: Vec<String>,
    /// Extra ssh options (e.g. `-J jump`).
    pub ssh_args: Vec<String>,
    /// Pipe ssh's stderr (its own errors and the server's logs) back to the
    /// caller instead of inheriting it; for an app with no terminal.
...
pub struct SshStream {
    ...
    /// Set when the target asked for `capture_stderr`.
...
/// client's terminal, unless the target captures it.
pub fn spawn_ssh(target: &SshTarget) -> std::io::Result<SshStream> {
    ...
    cmd.stderr(if target.capture_stderr {
