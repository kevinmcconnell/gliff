...
impl Pass {
    fn new(gpu: &Arc<Gpu>, spirv: &[u8], sampled: u32, storage: u32) -> Result<Self> {
        ...
        unsafe {
            ...
            let bindings: Vec<vk::DescriptorSetLayoutBinding> = (0..sampled + storage)
                .map(|i| {
                    let b = vk::DescriptorSetLayoutBinding::default().binding(i).descriptor_count(1).stage_flags(vk::ShaderStageFlags::COMPUTE);
                    if i < sampled {
                        b.descriptor_type(vk::DescriptorType::COMBINED_IMAGE_SAMPLER).immutable_samplers(&samplers)
                    } else {
                        b.descriptor_type(vk::DescriptorType::STORAGE_IMAGE)
                    }
                })
                .collect();
            let set_layout = gpu.device.create_descriptor_set_layout(&vk::DescriptorSetLayoutCreateInfo::default().bindings(&bindings), None)?;
            let push = [vk::PushConstantRange::default().stage_flags(vk::ShaderStageFlags::COMPUTE).size(std::mem::size_of::<Push>() as u32)];
            let set_layouts = [set_layout];
            let layout = gpu.device.create_pipeline_layout(&vk::PipelineLayoutCreateInfo::default().set_layouts(&set_layouts).push_constant_ranges(&push), None)?;
            let code = ash::util::read_spv(&mut std::io::Cursor::new(spirv))?;
            let module = gpu.device.create_shader_module(&vk::ShaderModuleCreateInfo::default().code(&code), None)?;
            ...
            let pipeline = gpu.device.create_compute_pipelines(vk::PipelineCache::null(), &[info], None).map_err(|(_, e)| e)?[0];
            gpu.device.destroy_shader_module(module, None);
            let sizes = [
                vk::DescriptorPoolSize { ty: vk::DescriptorType::COMBINED_IMAGE_SAMPLER, descriptor_count: sampled.max(1) * 4 },
                vk::DescriptorPoolSize { ty: vk::DescriptorType::STORAGE_IMAGE, descriptor_count: storage.max(1) * 4 },
            ];
            let pool = gpu.device.create_descriptor_pool(&vk::DescriptorPoolCreateInfo::default().max_sets(4).pool_sizes(&sizes), None)?;
            Ok(Self { gpu: gpu.clone(), sampler, set_layout, layout, pipeline, pool, sampled, storage })
        }
    }
    ...
    fn dispatch(&self, cmd: vk::CommandBuffer, views: &[vk::ImageView], width: u32, height: u32, has_aux: bool) -> Result<()> {
        ...
        unsafe {
            ...
            let set = dev.allocate_descriptor_sets(&vk::DescriptorSetAllocateInfo::default().descriptor_pool(self.pool).set_layouts(&layouts))?[0];
            let infos: Vec<[vk::DescriptorImageInfo; 1]> = views
                .iter()
                .enumerate()
                .map(|(i, &v)| {
                    let layout = if (i as u32) < self.sampled { vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL } else { vk::ImageLayout::GENERAL };
                    [vk::DescriptorImageInfo { sampler: self.sampler, image_view: v, image_layout: layout }]
                })
                .collect();
            let writes: Vec<vk::WriteDescriptorSet> = infos
                ...
                .enumerate()
                .map(|(i, info)| {
                    let ty = if (i as u32) < self.sampled { vk::DescriptorType::COMBINED_IMAGE_SAMPLER } else { vk::DescriptorType::STORAGE_IMAGE };
                    vk::WriteDescriptorSet::default().dst_set(set).dst_binding(i as u32).descriptor_type(ty).image_info(info)
                })
                .collect();
            ...
            let push = Push { width: width as i32, height: height as i32, has_aux: has_aux as i32 };
            ...
            dev.cmd_dispatch(cmd, width.div_ceil(2).div_ceil(16), height.div_ceil(2).div_ceil(16), 1);
        }
        Ok(())
    }
}
...
impl Split {
    ...
    pub(crate) fn record(&self, cmd: vk::CommandBuffer, src: &Image, main: &Image, aux: Option<&Image>, width: u32, height: u32) -> Result<()> {
        let aux_views = aux.map(|a| (a.plane_views[0], a.plane_views[1])).unwrap_or((main.plane_views[0], main.plane_views[1]));
        let views = [src.view0(), main.plane_views[0], main.plane_views[1], aux_views.0, aux_views.1];
...
impl Recombine {
    ...
    pub(crate) fn record(&self, cmd: vk::CommandBuffer, main: &Image, aux: Option<&Image>, dst: &Image, width: u32, height: u32) -> Result<()> {
        let aux_views = aux.map(|a| (a.plane_views[0], a.plane_views[1])).unwrap_or((main.plane_views[0], main.plane_views[1]));
        let views = [main.plane_views[0], main.plane_views[1], aux_views.0, aux_views.1, dst.view0()];
        self.0.dispatch(cmd, &views, width, height, aux.is_some())
    }
}
