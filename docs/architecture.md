...
| Crate | Role | `unsafe` |
|---|---|---|
| `haver-proto` | wire messages, framing header, CPU reference for colour and the AVC444 split/recombine | none |
...
| `haver-vk` | Vulkan device, dmabuf import/export, split/recombine compute, H.264 encode/decode, header parser | **yes, isolated here** |
| `haver-server` | ties capture+input+encoder to the protocol; `--stdio`/`--listen` | none |
| `haver-client` | GTK4/libadwaita UI, decode worker | one block: hands GTK a dmabuf fd |
| `haver-probe` | environment checks and the headless test client | none |

`haver-vk` wraps `ash`, whose every call is `unsafe` because Vulkan is a C API
...
- **4:4:4 by two 4:2:0 streams (AVC444).** Hardware H.264 encoders only do
  ...
  `haver-proto::chroma` is unit-tested through 1080p and is the oracle the
  compute shaders are checked against (`haver-probe pipeline`).
...
- **Threading.** Each pipeline lives on one thread: the server loop and the
  ...
  `haver-vk` objects. Capture and input each own a Wayland connection on their
...
- **Unit tests** cover the pure logic: AVC444 split/recombine losslessness,
  ...
  discovery.
- **`haver-probe`** is the hardware integration harness: `protocols`,
...
Dual420, from `haver-probe roundtrip`:
...
The binaries are dynamically linked: `haver-server`/`haver-probe` need
