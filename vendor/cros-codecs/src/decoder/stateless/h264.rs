    ...
    H: DecodedHandle + Clone,
{
    ...
    fn bump_over_reorder_limit(&mut self) -> impl Iterator<Item = H> {
        self.dpb.bump_over_reorder_limit().into_iter().flatten()
    }
    ...
    B::Handle: Clone,
{
    ...
    fn finish_picture(&mut self, pic: CurrentPicState<B::Picture>) -> anyhow::Result<()> {
        ...
        self.ready_queue.extend(self.codec.bump_over_reorder_limit());
    ...
    fn process_nalu(
        ...
        match nalu.header.type_ {
            ...
            NaluType::AuDelimiter => {
                // An access unit delimiter closes the picture in progress, so a
                // low-latency sender can get every frame out without waiting
                // for the next one.
                if let Some(cur_pic) = self.codec.current_pic.take() {
                    self.finish_picture(cur_pic)?;
                }
