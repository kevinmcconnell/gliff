...
pub(crate) fn with_h264_profile<R>(encode: bool, f: impl FnOnce(&vk::VideoProfileInfoKHR) -> R) -> R {
    let mut enc = vk::VideoEncodeH264ProfileInfoKHR::default().std_profile_idc(std_video::StdVideoH264ProfileIdc_STD_VIDEO_H264_PROFILE_IDC_HIGH);
    ...
    let profile = if encode {
        base.video_codec_operation(vk::VideoCodecOperationFlagsKHR::ENCODE_H264).push_next(&mut enc).push_next(&mut usage)
    } else {
        base.video_codec_operation(vk::VideoCodecOperationFlagsKHR::DECODE_H264).push_next(&mut dec)
    };
    f(&profile)
}
...
pub(crate) fn std_header(encode: bool) -> vk::ExtensionProperties {
    ...
    let name = if encode { STD_ENCODE_NAME } else { STD_DECODE_NAME };
...
impl Session {
    pub(crate) fn new(gpu: &Arc<Gpu>, info: &vk::VideoSessionCreateInfoKHR) -> Result<Self> {
        ...
        unsafe {
            ...
            (gpu.video.fp().create_video_session_khr)(gpu.device.handle(), info, std::ptr::null(), &mut handle).result()?;
            let mut count = 0u32;
            (gpu.video.fp().get_video_session_memory_requirements_khr)(gpu.device.handle(), handle, &mut count, std::ptr::null_mut()).result()?;
            let mut reqs = vec![vk::VideoSessionMemoryRequirementsKHR::default(); count as usize];
            (gpu.video.fp().get_video_session_memory_requirements_khr)(gpu.device.handle(), handle, &mut count, reqs.as_mut_ptr()).result()?;
            let mut memory = Vec::new();
            let mut binds = Vec::new();
            for r in &reqs {
                let mem = gpu.allocate(r.memory_requirements, vk::MemoryPropertyFlags::empty(), None)?;
                memory.push(mem);
                binds.push(vk::BindVideoSessionMemoryInfoKHR::default().memory_bind_index(r.memory_bind_index).memory(mem).memory_offset(0).memory_size(r.memory_requirements.size));
            }
            (gpu.video.fp().bind_video_session_memory_khr)(gpu.device.handle(), handle, binds.len() as u32, binds.as_ptr()).result()?;
            Ok(Self { gpu: gpu.clone(), handle, memory })
        }
    }
}
...
impl Drop for Session {
    fn drop(&mut self) {
        ...
        unsafe {
            (self.gpu.video.fp().destroy_video_session_khr)(self.gpu.device.handle(), self.handle, std::ptr::null());
...
impl SessionParameters {
    pub(crate) fn new(gpu: &Arc<Gpu>, info: &vk::VideoSessionParametersCreateInfoKHR) -> Result<Self> {
        let mut handle = vk::VideoSessionParametersKHR::null();
        // SAFETY: valid create info.
        unsafe { (gpu.video.fp().create_video_session_parameters_khr)(gpu.device.handle(), info, std::ptr::null(), &mut handle).result()? };
        Ok(Self { gpu: gpu.clone(), handle })
    }
}
...
impl Drop for SessionParameters {
    fn drop(&mut self) {
        // SAFETY: owners wait for GPU work before dropping.
        unsafe { (self.gpu.video.fp().destroy_video_session_parameters_khr)(self.gpu.device.handle(), self.handle, std::ptr::null()) };
    }
}
...
impl H264Encoder {
    pub fn new(gpu: &Arc<Gpu>, settings: EncoderSettings) -> Result<Self> {
        ...
        let queue = gpu.encode_queue.ok_or_else(|| Error::Unsupported("no encode queue".into()))?;
        let coded = vk::Extent2D { width: settings.coded_width(), height: settings.coded_height() };
        with_h264_profile(true, |profile| {
            ...
            let mut caps = vk::VideoCapabilitiesKHR::default().push_next(&mut enc_caps).push_next(&mut h264_caps);
            // SAFETY: valid physical device and chained structs.
            unsafe { (gpu.video_instance.fp().get_physical_device_video_capabilities_khr)(gpu.physical, profile, &mut caps).result()? };
            if coded.width > caps.max_coded_extent.width || coded.height > caps.max_coded_extent.height {
                return Err(Error::Unsupported(format!("{}x{} exceeds the encoder maximum {:?}", coded.width, coded.height, caps.max_coded_extent)));
            }
            if !enc_caps.rate_control_modes.contains(vk::VideoEncodeRateControlModeFlagsKHR::CBR) {
            ...
            let spss = [sps];
            let ppss = [pps];
            let add = vk::VideoEncodeH264SessionParametersAddInfoKHR::default().std_sp_ss(&spss).std_pp_ss(&ppss);
            let mut h264_params = vk::VideoEncodeH264SessionParametersCreateInfoKHR::default().max_std_sps_count(1).max_std_pps_count(1).parameters_add_info(&add);
            let params_info = vk::VideoSessionParametersCreateInfoKHR::default().video_session(session.handle).push_next(&mut h264_params);
            let params = SessionParameters::new(gpu, &params_info)?;

            let dpb = Image::nv12(gpu, Role::EncodeDpb, coded.width, coded.height, 2, Some(profile))?;
            let size = (coded.width * coded.height * 2) as usize;
            let bitstream = HostBuffer::new(gpu, size.max(1 << 20), vk::BufferUsageFlags::VIDEO_ENCODE_DST_KHR, Some(profile))?;
            ...
            let query_info = vk::QueryPoolCreateInfo::default().query_type(vk::QueryType::VIDEO_ENCODE_FEEDBACK_KHR).query_count(1).push_next(&mut feedback).push_next(&mut profile_copy);
            ...
            tracing::debug!(sps = enc.sps.len(), pps = enc.pps.len(), "encoder parameter sets");
            Ok(enc)
        })
    }
    ...
    pub fn new_input(&self) -> Result<Image> {
        with_h264_profile(true, |profile| Image::nv12(&self.gpu, Role::EncodeSource, self.settings.coded_width(), self.settings.coded_height(), 1, Some(profile)))
    }

    fn encoded_parameters(&self, sps: bool, pps: bool) -> Result<Vec<u8>> {
        let mut h264 = vk::VideoEncodeH264SessionParametersGetInfoKHR::default().write_std_sps(sps).write_std_pps(pps).std_sps_id(0).std_pps_id(0);
        let info = vk::VideoEncodeSessionParametersGetInfoKHR::default().video_session_parameters(self.params.handle).push_next(&mut h264);
        let fp = self.gpu.encode.fp().get_encoded_video_session_parameters_khr;
        // SAFETY: two-call size query then fill, on a valid parameters object.
        let mut data = unsafe {
            let mut size = 0usize;
            fp(self.gpu.device.handle(), &info, std::ptr::null_mut(), &mut size, std::ptr::null_mut()).result()?;
            let mut data = vec![0u8; size];
            fp(self.gpu.device.handle(), &info, std::ptr::null_mut(), &mut size, data.as_mut_ptr().cast()).result()?;
            data.truncate(size);
            data
        };
    ...
    /// is ready.
    pub fn encode(&mut self, input: &Image, timeline: &Timeline, wait: Option<u64>, force_keyframe: bool) -> Result<EncodedPacket> {
        ...
        let current = SlotPicture { frame_num: self.frame_num, poc: self.poc, idr };
        ...
        with_h264_profile(true, |_profile| {
            ...
            let ref_std = |p: SlotPicture| {
                ...
                std_video::StdVideoEncodeH264ReferenceInfo {
                    flags,
                    primary_pic_type: if p.idr { std_video::StdVideoH264PictureType_STD_VIDEO_H264_PICTURE_TYPE_IDR } else { std_video::StdVideoH264PictureType_STD_VIDEO_H264_PICTURE_TYPE_P },
            ...
            let mut begin_slots = vec![vk::VideoReferenceSlotInfoKHR::default().slot_index(-1).picture_resource(&setup_res)];
            let ref_std_info = ref_slot.map(|r| ref_std(self.slots[r].expect("active reference has a picture")));
            let mut ref_dpb_info = ref_std_info.as_ref().map(|s| vk::VideoEncodeH264DpbSlotInfoKHR::default().std_reference_info(s));
            if let (Some(r), Some(res), Some(dpb_info)) = (ref_slot, ref_res.as_ref(), ref_dpb_info.as_mut()) {
                begin_slots.push(vk::VideoReferenceSlotInfoKHR::default().slot_index(r as i32).picture_resource(res).push_next(dpb_info));
            }

            let setup_std = ref_std(current);
            let mut setup_dpb_info = vk::VideoEncodeH264DpbSlotInfoKHR::default().std_reference_info(&setup_std);
            let setup_ref = vk::VideoReferenceSlotInfoKHR::default().slot_index(setup_slot as i32).picture_resource(&setup_res).push_next(&mut setup_dpb_info);
            let mut ref_dpb_info2 = ref_std_info.as_ref().map(|s| vk::VideoEncodeH264DpbSlotInfoKHR::default().std_reference_info(s));
            let encode_refs: Vec<vk::VideoReferenceSlotInfoKHR> = match (ref_slot, ref_res.as_ref(), ref_dpb_info2.as_mut()) {
                (Some(r), Some(res), Some(info)) => vec![vk::VideoReferenceSlotInfoKHR::default().slot_index(r as i32).picture_resource(res).push_next(info)],
                _ => Vec::new(),
            };
            ...
            let pic_type = if idr { std_video::StdVideoH264PictureType_STD_VIDEO_H264_PICTURE_TYPE_IDR } else { std_video::StdVideoH264PictureType_STD_VIDEO_H264_PICTURE_TYPE_P };
            ...
            let slices = [vk::VideoEncodeH264NaluSliceInfoKHR::default().constant_qp(0).std_slice_header(&std_slice)];
            let mut h264_pic = vk::VideoEncodeH264PictureInfoKHR::default().nalu_slice_entries(&slices).std_picture_info(&std_pic).generate_prefix_nalu(false);

            let src = vk::VideoPictureResourceInfoKHR::default().coded_extent(vk::Extent2D { width: w, height: h }).base_array_layer(0).image_view_binding(input.view0());
            ...
            let mut begin = vk::VideoBeginCodingInfoKHR::default().video_session(self.session.handle).video_session_parameters(self.params.handle).reference_slots(&begin_slots);
            if self.started {
            ...
            let dpb = &self.dpb;
            self.commands.run(timeline.semaphore, wait, None, true, |cmd| {
                ...
                unsafe {
                    ...
                    if !started {
                        ...
                        let control = vk::VideoCodingControlInfoKHR::default()
                            .flags(vk::VideoCodingControlFlagsKHR::RESET | vk::VideoCodingControlFlagsKHR::ENCODE_RATE_CONTROL | vk::VideoCodingControlFlagsKHR::ENCODE_QUALITY_LEVEL)
        ...
        unsafe {
            self.gpu.device.get_query_pool_results(self.query_pool, 0, &mut results, vk::QueryResultFlags::WAIT | vk::QueryResultFlags::WITH_STATUS_KHR)?;
        }
        ...
        self.poc += 2;
        self.started = true;
        Ok(EncodedPacket { keyframe: idr, data })
    }
}
...
impl RateControl {
    ...
    fn infos(&mut self) -> (vk::VideoEncodeRateControlInfoKHR<'_>, vk::VideoEncodeH264RateControlInfoKHR<'_>) {
        self.layers[0].p_next = (&mut self.h264_layer as *mut vk::VideoEncodeH264RateControlLayerInfoKHR).cast();
        ...
        let h264 = vk::VideoEncodeH264RateControlInfoKHR::default()
            .flags(vk::VideoEncodeH264RateControlFlagsKHR::REGULAR_GOP | vk::VideoEncodeH264RateControlFlagsKHR::REFERENCE_PATTERN_FLAT)
            .gop_frame_count(60)
            .idr_period(0)
...
fn std_parameter_sets(s: &EncoderSettings, max_level: std_video::StdVideoH264LevelIdc) -> (std_video::StdVideoH264SequenceParameterSet, std_video::StdVideoH264PictureParameterSet) {
    ...
    let sps = std_video::StdVideoH264SequenceParameterSet {
        ...
        chroma_format_idc: std_video::StdVideoH264ChromaFormatIdc_STD_VIDEO_H264_CHROMA_FORMAT_IDC_420,
        seq_parameter_set_id: 0,
    ...
    let pps = std_video::StdVideoH264PictureParameterSet {
        ...
        weighted_bipred_idc: std_video::StdVideoH264WeightedBipredIdc_STD_VIDEO_H264_WEIGHTED_BIPRED_IDC_DEFAULT,
