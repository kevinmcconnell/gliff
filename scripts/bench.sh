...
#
# Link shaping, by environment:
#   BENCH_MBIT, BENCH_DELAY_MS, BENCH_QUEUE_KB   TCP proxy (rate, one-way delay)
#   BENCH_LOSS, BENCH_NETEM_MS [, BENCH_MBIT]    netem on a private loopback:
#       packet loss in percent and one-way delay, for TCP and UDP alike
#       (scripts/netem.sh; no root needed)
set -uo pipefail
cd "$(dirname "$0")/.."
if [ -n "${BENCH_LOSS:-}${BENCH_NETEM_MS:-}" ] && [ -z "${NETEM_INSIDE:-}" ]; then
    exec scripts/netem.sh "${BENCH_NETEM_MS:-0}" "${BENCH_LOSS:-0}" ${BENCH_MBIT:+"$BENCH_MBIT"} -- "$0" "$@"
fi
...
if [ -n "${NETEM_INSIDE:-}" ]; then
    echo "  link: netem ${BENCH_NETEM_MS:-0} ms one-way, ${BENCH_LOSS:-0}% loss${BENCH_MBIT:+, $BENCH_MBIT Mbit/s}"
elif [ -n "${BENCH_MBIT:-}" ]; then
    CPORT=$((PORT + 1))
